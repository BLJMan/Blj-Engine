...
class ModchartState 
{
    ...
    function new()
    {
				...
				setVar("downscroll", CoolThings.downscroll);
				setVar("toggleDownscroll", false);
				...
				Lua_helper.add_callback(lua,"tweenPosOut", function(id:String, toX:Int, toY:Int, time:Float, delay:Float, onComplete:String) {
					FlxTween.tween(getActorByName(id), {x: toX, y: toY}, time, {ease: FlxEase.cubeOut, startDelay: delay, onComplete: function(flxTween:FlxTween) { if (onComplete != '' && onComplete != null) {callLua(onComplete,[id]);}}});
				});
