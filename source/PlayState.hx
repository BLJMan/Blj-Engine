...
import Shaders.DistortBGShader;
...
class PlayState extends MusicBeatState
{
	...
	private var executeModchart:Bool = false;
	...
	public var dummyEvents:Array<Dynamic> = [];
	...
	private var camFollowThing:FlxObject;

	private static var prevCamFollow:FlxPoint;
	private static var prevCamFollowThing:FlxObject;
	...
	public static var creditMod:String;
	...
	var marioDancer1:BackgroundMarios;
	var marioDancer2:BackgroundMarios;
	var iHateYou:FlxSprite;

	var goldenLandP2BG:FlxSprite; 
	var goldenLandP2Floor:FlxSprite; 
	var goldenLandP2Floor2:FlxSprite;

	private var zoomBack:Bool = false;
	private var camZooming:Bool = false;
	private var reverseZooming:Bool = false;
	private var curSong:String = CoolThings.playstateSong;
	...
	public static var fakeSpeed:Float;
	...
	var preloadedCharacters:Int = 0;

	var curDad:Character;
	var curBF:Character;

	public static var songHasDialogue = false;
	...
	override public function create()
	{
		...
		if (SONG.song != curSong && !CoolThings.persist)
		{
			Main.dumpCache();
		}

		//PRELOADING CHARACTERS YAYAYAYAY
		if (SONG.song.toLowerCase() == "triple-trouble")
		{
			preloadCharacter("beast");
			preloadCharacter("knucks");
			preloadCharacter("egg");
			preloadCharacter("tails");
			preloadCharacter("bfTT");
		}

		if (SONG.song == "challengedd")
		{
			var dadA = new Character(61.15, -94.75, 'eduardo');
			dadA.alpha = 0.00001;
			add(dadA);
		}
		...
		switch (SONG.song.toLowerCase()) //custom arrow colors
		{
			...
			case "relighted": 
				CoolUtil.arrowHSV = [[-50, 0, 0], [100, -40, 0], [-170, -40, -10], [-90, -10, 0]];
		...
		fakeSpeed = SONG.speed; //used for change speed event i think

		if (SONG.events == null)
		{
			//SONG.events = [new SongEvent(0, SONG.bpm, null, "BPMChange", "0,0")];//idk for testing i suppose
		}
		...
		#if windows
		songHasDialogue = FileSystem.exists(Paths.txt(PlayState.SONG.song.toLowerCase()  + "/dialog"));

		if (songHasDialogue)
			dialogue = CoolUtil.coolTextFile(Paths.txt(PlayState.SONG.song.toLowerCase()  + "/dialog"));
		...
		{
			curStage = SONG.stage;//nice
		}

		if (SONG.song == "challengedd")
		{
			dad2 = new Character(-930, 95, "eduardo"); //well well well
		}
		...
		switch (curStage)
		{
			...
			case 'sonicfunStage':
			{
				...
				funpillarts2ANIM = new FlxSprite(182, -100);
			...
			case 'troubleStage': 
			{
				...
				var sSKY:FlxSprite = new FlxSprite(-621.1, -395.65).loadGraphic(Paths.image('triple-trouble/Glitch'));
				sSKY.antialiasing = true;
				...
				p3staticbg.alpha = 0.00001;
				add(p3staticbg);

				var trees:FlxSprite = new FlxSprite(-607.35, -401.55).loadGraphic(Paths.image('triple-trouble/Trees'));
				trees.antialiasing = true;
				...
				var bg2:FlxSprite = new FlxSprite(-623.5, -410.4).loadGraphic(Paths.image('triple-trouble/Trees2'));
				bg2.updateHitbox();
				bg2.antialiasing = true;
				...
				var bg:FlxSprite = new FlxSprite(-630.4, -266).loadGraphic(Paths.image('triple-trouble/Grass'));
				bg.antialiasing = true;
			...
			case "gamble": 
			{
				...
				var table = new FlxSprite(-1000, -420).loadGraphic(Paths.image("table"));
				table.scrollFactor.set(1, 1);
				table.antialiasing = true;
				table.scale.set(1.1, 1.1);

				cover = new FlxSprite(610, -420).loadGraphic(Paths.image("table"));
				cover.scrollFactor.set(1, 1);
				cover.antialiasing = true;
				cover.scale.set(1.1, 1.1);

				gambleLight = new FlxSprite(-1180, -210).loadGraphic(Paths.image("light"));
				gambleLight.scrollFactor.set(1, 1);
				gambleLight.antialiasing = true;
				gambleLight.alpha = 0.375;
			...
			case "well": 
			{
				...
				var floor = new FlxSprite(-1700, -700).loadGraphic(Paths.image("edd/HousesAndFloor"));
				var second = new FlxSprite(-1600, -700).loadGraphic(Paths.image("edd/SecondParalax"));
				var bg = new FlxSprite(-1600, -700).loadGraphic(Paths.image("edd/SkyBox"));
				var fence = new FlxSprite(-1700, -700).loadGraphic(Paths.image("edd/Fence"));
				var clouds = new FlxSprite(-1700, -700).loadGraphic(Paths.image("edd/Clouds"));
				floor.scrollFactor.set(0.9, 0.9);
				bg.scrollFactor.set(0.2, 0.2);
				second.scrollFactor.set(0.4, 0.4);
				fence.scrollFactor.set(0.9, 0.9);
				clouds.scrollFactor.set(0.8, 0.8);
				bg.antialiasing = CoolThings.antialiasing;
				second.antialiasing = CoolThings.antialiasing;
				floor.antialiasing = CoolThings.antialiasing;
				fence.antialiasing = CoolThings.antialiasing;
				clouds.antialiasing = CoolThings.antialiasing;
				add(bg);
				add(second);
				add(clouds);
				add(floor);
			...
			case "gasterStage": 
			{
				...
				var bg = new FlxSprite(-700, -210).loadGraphic(Paths.image("overwrite_bg"));
				bg.scrollFactor.set(0.95, 0.95);
				bg.antialiasing = CoolThings.antialiasing;
				add(bg);
				light = new FlxSprite(-700, -210).loadGraphic(Paths.image("overwrite_light"));
				light.scrollFactor.set(0.95, 0.95);
				light.antialiasing = CoolThings.antialiasing;
				
				var fire = new FlxSprite(-500, 400);
				fire.frames = Paths.getSparrowAtlas('rpgfire');
				fire.animation.addByPrefix("idle", "fire_iddle0");
				fire.antialiasing = CoolThings.antialiasing;
				add(fire);
				fire.animation.play("idle");

				var fire2 = new FlxSprite(-100, 300);
				fire2.frames = Paths.getSparrowAtlas('rpgfire');
				fire2.animation.addByPrefix("idle", "fire_iddle0");
				fire2.antialiasing = CoolThings.antialiasing;
				add(fire2);
				fire2.animation.play("idle");

				var fire3 = new FlxSprite(300, 200);
				fire3.frames = Paths.getSparrowAtlas('rpgfire');
				fire3.animation.addByPrefix("idle", "fire_iddle0");
				fire3.antialiasing = CoolThings.antialiasing;
				add(fire3);
				fire3.animation.play("idle");

				var fire4 = new FlxSprite(700, 200);
				fire4.frames = Paths.getSparrowAtlas('rpgfire');
				fire4.animation.addByPrefix("idle", "fire_iddle0");
				fire4.antialiasing = CoolThings.antialiasing;
				add(fire4);
				fire4.animation.play("idle");

				var fire5 = new FlxSprite(1100, 300);
				fire5.frames = Paths.getSparrowAtlas('rpgfire');
				fire5.animation.addByPrefix("idle", "fire_iddle0");
				fire5.antialiasing = CoolThings.antialiasing;
				add(fire5);
				fire5.animation.play("idle");

				var fire6 = new FlxSprite(1500, 400);
				fire6.frames = Paths.getSparrowAtlas('rpgfire');
				fire6.animation.addByPrefix("idle", "fire_iddle0");
				fire6.antialiasing = CoolThings.antialiasing;
				add(fire6);
				fire6.animation.play("idle");			
			}
			...
			case "i hate you": 
			{
				...
				var bg = new FlxSprite(-200, 0).loadGraphic(Paths.image("IHY/Luigi_IHY_Background_Assets_ete_sech"));
				bg.scale.set(1.2, 1.2);
				bg.scrollFactor.set(0.65, 0.65);
				bg.antialiasing = CoolThings.antialiasing;
				add(bg);

				iHateYou = new FlxSprite(1300, 300).loadGraphic(Paths.image("IHY/Luigi_IHY_Background_Assets_ete_sech_v2"));
				iHateYou.alpha = 0.00001;
				iHateYou.scrollFactor.set(0.65, 0.65);
				iHateYou.antialiasing = CoolThings.antialiasing;
				iHateYou.scale.set(0.8, 0.8);
				add(iHateYou);

				marioDancer1 = new BackgroundMarios(0, 520);
				marioDancer1.alpha = 0.00001;
				marioDancer1.scrollFactor.set(1, 1);
 
				marioDancer2 = new BackgroundMarios(1300, 520);
				marioDancer2.alpha = 0.00001;
				marioDancer2.scrollFactor.set(1, 1);

				var floor = new FlxSprite(-400, -30).loadGraphic(Paths.image("IHY/Luigi_IHY_Background_Assets_Floor"));
				floor.scrollFactor.set(1, 1);
				floor.antialiasing = CoolThings.antialiasing;
				add(floor);

				add(marioDancer1);
				marioDancer1.animation.play("dance");
				add(marioDancer2);
				marioDancer2.animation.play("dance");
			}
			...
			case "mario-land": 
			{
				...
				var bg = new FlxSprite(200, 40).loadGraphic(Paths.image("mario-land/Nubes"));
				bg.scrollFactor.set(0.7, 0.7);
				bg.scale.set(2.3, 2.3);
				bg.antialiasing = false;
				add(bg);

				goldenLandP2BG = new FlxSprite();
				goldenLandP2BG.frames = Paths.getSparrowAtlas('mario-land/Mario_Phase2_Background_Assets_Static');
				goldenLandP2BG.animation.addByPrefix("idle", "Estatica papu instancia 1");
				goldenLandP2BG.alpha = 0.00001;
				goldenLandP2BG.setPosition(bg.x + 30, bg.y);
				goldenLandP2BG.scale.set(2.8, 2.8);
				goldenLandP2BG.scrollFactor.set(0.7, 0.7);
				goldenLandP2BG.antialiasing = false;
				add(goldenLandP2BG);
				goldenLandP2BG.animation.play("idle");//i am so tired rn

				var floor = new FlxSprite(220, 410).loadGraphic(Paths.image("mario-land/Nubes2"));
				floor.scrollFactor.set(0.73, 0.73);
				floor.scale.set(2.3, 2.3);
				floor.antialiasing = false;
				add(floor);

				goldenLandP2Floor = new FlxSprite(220, 410).loadGraphic(Paths.image("mario-land/weaspuki2"));
				goldenLandP2Floor.scrollFactor.set(0.73, 0.73);
				goldenLandP2Floor.scale.set(2.3, 2.3);
				goldenLandP2Floor.alpha = 0.00001;
				goldenLandP2Floor.antialiasing = false;
				add(goldenLandP2Floor);

				var floor2 = new FlxSprite(270, 435).loadGraphic(Paths.image("mario-land/Nubes3"));
				floor2.scrollFactor.set(1, 1);
				floor2.scale.set(2.3, 2.3);
				floor2.antialiasing = false;
				add(floor2);

				goldenLandP2Floor2 = new FlxSprite(270, 435).loadGraphic(Paths.image("mario-land/weaspuki1"));
				goldenLandP2Floor2.scrollFactor.set(1, 1);
				goldenLandP2Floor2.scale.set(2.3, 2.3);
				goldenLandP2Floor2.antialiasing = false;
				goldenLandP2Floor2.alpha = 0.00001;
				add(goldenLandP2Floor2);

				var blocks = new FlxSprite(600, -160).loadGraphic(Paths.image("mario-land/Nubes4"));
				blocks.scrollFactor.set(1, 1);
				blocks.scale.set(2.9, 2.9);
		...
		if (SONG.song == "relighted")
			gf.scale.set(0.55, 0.55);//she's far away

		dad = new Character(100, 100, SONG.player2);

		if (SONG.song.toLowerCase() == "relighted")
			dad2 = new Character(150, 100, "xgaster2");
		...
		switch (curStage)
		{
			...
			case "well": 
				boyfriend.x += 500;
			case "gamble": 
				boyfriend.setPosition(97, 250);
				dad.setPosition(-672, 100);
				gf.setPosition(400, -75);
			case "i hate you": 
				boyfriend.x += 180;
				dad.y += 130;
			case "mario-land": 
		...
		if (curStage == "gasterStage")
		{
			add(light);
			FlxTween.tween(light, {alpha: 0}, 2.9, {ease: FlxEase.quadInOut, type: PINGPONG});
		}
		...
		if (SONG.song.toLowerCase() == "onslaught" || SONG.song.toLowerCase() == "run")
		{
			SONG.gfVisible = false;
		}
		...
		if (SONG.gfVisible != true && SONG.gfVisible != false)//wtf why do I have to do this instead of "== null" fuck you haxe
			SONG.gfVisible = true;
		 
		gf.visible = SONG.gfVisible;
		...
		grayThingy.makeGraphic(FlxG.width + 400, 300, FlxColor.BLACK);
		grayThingy.alpha = 0.35;
		grayThingy.screenCenter(X);
		grayThingy.scrollFactor.set();
		add(grayThingy);

		if (creditMod == null) {creditMod = "unknown";}

		songNameBox = new FlxSprite(-300, 160).loadGraphic(Paths.image("UI/creditsBox", "shared"), false);
		//songNameBox = new FlxSprite(-200, 690).makeGraphic(SONG.song.length * 12, 30, FlxColor.GRAY);
		songNameBox.cameras = [camHUD];
		songNameBox.scale.set(3, 3);
		songNameBox.updateHitbox();
		songNameBox.alpha = 0.00001;
		songNameBox.antialiasing = false;

		textThing = new FlxText(songNameBox.x + 10, songNameBox.y + 15, 0, SONG.song + "\n" + creditMod);
		textThing.alignment = CENTER;
		textThing.cameras = [camHUD];
		textThing.scale.set(2.2, 2.2);
		textThing.color = FlxColor.BLACK;
		textThing.alpha = 0.00001;
		textThing.antialiasing = false;
		...
		strumLine = new FlxSprite(0, 50).makeGraphic(FlxG.width, 10, FlxColor.RED);
		strumLine.scrollFactor.set();

		timeTxt = new FlxText(0, strumLine.y + 40, 400, "", 40);
		...
		botTxt = new FlxText(0, strumLine.y + 100, 400, "BOTPLAY", 40);
		...
		camFollowThing.setPosition(camPos.x, camPos.y);
		...
		if (prevCamFollowThing != null)
		{
			camFollowThing = prevCamFollowThing;
			prevCamFollowThing = null;
		}

		//add(camFollow);
		add(camFollowThing);

		FlxG.camera.follow(camFollowThing, LOCKON, 1);
			...
			switch (curSong.toLowerCase())
			{
				case "winter-horrorland":
					...
					new FlxTimer().start(0.1, function(tmr:FlxTimer)
					{
						...
						camFollow.y = -2050;
						camFollow.x += 200;
						camFollowThing.x += 200;
						camFollowThing.y = -2050;
						FlxG.camera.focusOn(camFollow);
		...
		if (SONG.song == "pasta-night")
		{
			FlxTween.tween(gambleLight, {x: gambleLight.x}, 0.3, {ease: FlxEase.sineInOut, onComplete: onLightTweenCompleteRight});
		}
	...
	{
		FlxTween.tween(songNameBox, {x: -90, alpha: 1}, 0.5, {ease: FlxEase.circOut, startDelay: 0.2});
		FlxTween.tween(textThing, {x: 80, alpha: 1}, 0.6, {ease: FlxEase.circOut, startDelay: 0.2});
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			switch (swagCounter)
			{
				...
				case 3:
					...
					if (!CoolUtil.ogSongs.contains(SONG.song.toLowerCase()))
						spawnSongName();
				case 4:
			}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		CoolThings.playstateSong = curSong;
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(SONG.speed, 2)), daNoteData, oldNote, true, 0, false);
					totalNotes ++;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			{
				cpuStrums.forEach(function(leNote:StrumNote)
				{
					leNote.x = - 1000; //should be off screen?? lol
				});
	...
	{ 
		...
		FlxG.watch.addQuick("curDad", curDad);
		FlxG.watch.addQuick("curBF", curBF);*/
		#if debug
		#if !FREEPLAY

		FlxG.watch.addQuick("camera", camFollow.x + " | " + camFollow.y + " | " + camFollowThing.x + " | " + camFollowThing.y);
		FlxG.watch.addQuick("aim", dad.animation.curAnim.curFrame + " | " + boyfriend.animation.curAnim.curFrame + " | " + gf.animation.curAnim.curFrame);
		FlxG.watch.addQuick("aimName", dad.animation.curAnim.name + " | " + boyfriend.animation.curAnim.name + " | " + gf.animation.curAnim.name);
		if (curSong == "pasta-night")
			FlxG.watch.addQuick("light", gambleLight.angle + " | " + gambleLight.x);
		FlxG.watch.addQuick("song", FlxG.sound.music.time + " | " + vocals.time);
		FlxG.watch.addQuick("hud", accuracy + " | " + misses + " | " + songScore);
		FlxG.watch.addQuick("camZoom", FlxG.camera.zoom + " | " + camHUD.zoom);
		FlxG.watch.addQuick("fakeSpeed", fakeSpeed);
		FlxG.watch.addQuick("fuck", grpNoteSplashes.members);
		#end
		#end

		accuracy = FlxMath.roundDecimal((songScore / ((noteHits + misses) * 350)) * 100, 2); 

		if (!CoolThings.displaceCam)
		{
			camDisplaceX = 0;
			camDisplaceY = 0;
		}
		...
		lerpScore = Math.floor(FlxMath.lerp(lerpScore, songScore, 0.15));

		camFollowThing.setPosition(FlxMath.lerp(camFollowThing.x, camFollow.x, CoolUtil.boundTo(elapsed * 2.6, 0, 1)), FlxMath.lerp(camFollowThing.y, camFollow.y, CoolUtil.boundTo(elapsed * 2.6, 0, 1)));
		...
		if (executeModchart && luaModchart != null)
		{
			...
			/*if (luaModchart.getVar("toggleDownscroll", "bool"))
			{
			...
			if (luaModchart.getVar("hideTimeAndRating",'bool'))
			{
				timeTxt.visible = false;
				noScore = true;
			}
		...
		scoreTxt.text = "Score:" + lerpScore + " (" + accuracy + "%)";
		...
		FlxG.camera.angle = FlxMath.lerp(0, FlxG.camera.angle, CoolUtil.boundTo(1 - (elapsed * 4), 0, 1));
		...
		var roundedSpeed:Float = FlxMath.roundDecimal(fakeSpeed, 2);
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				// i am so fucking sorry for this if condition -- what? -blj
				...
				if (daNote.mustPress && !daNote.modifiedByLua)
				{
					...
					else if (isPixelArrows && daNote.isSustainNote)
						daNote.x = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].x + 30;
					else if (!isPixelArrows && daNote.isSustainNote)
						daNote.x = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].x + 38;
				}
				else if (!daNote.wasGoodHit && !daNote.modifiedByLua)
				{
					if (!daNote.isSustainNote)
						...
						daNote.x = cpuStrums.members[Math.floor(Math.abs(daNote.noteData))].x + 30;
					else if (!isPixelArrows && daNote.isSustainNote)
						daNote.x = cpuStrums.members[Math.floor(Math.abs(daNote.noteData))].x + 38;
				}
		...
		if (!inCutscene)
		{
			if (!CoolThings.botplay)
			{
				...
				{
					if (CoolThings.hitsounds)
						FlxG.sound.play(Paths.sound('SNAP'));
					onKeyPress(pressArray);
				}
			...
			if (Conductor.songPosition >= Std.parseFloat(daPos[0]))
			{
				...
				switch (daType)
				{
					...
					{ 
						...
						switchCharacter(Std.string(daVal1), data[0]);
					...
					case "playAnim": 
					{
						...
						{
							case 1: 
								characterPlayAnim(dad, daVal2);
							case 2: 
								characterPlayAnim(boyfriend, daVal2);
							case 3: 
								characterPlayAnim(gf, daVal2);
						}
					...
					case "camFocus": 
					{
						...
						switch daVal1
						{
							case 0: 
							{
								camFollow.set(dad.getMidpoint().x + Std.parseFloat(data[0]), dad.getMidpoint().y + Std.parseFloat(data[1]));
								
								camForced = true;
							}
							case 1: 
							{
								camFollow.set(boyfriend.getMidpoint().x + Std.parseFloat(data[0]), boyfriend.getMidpoint().y + Std.parseFloat(data[1]));
							
								camForced = true;
							}
							case 2: 
							{
								camFollow.set(gf.getMidpoint().x + Std.parseFloat(data[0]), gf.getMidpoint().y + Std.parseFloat(data[1]));
	...
	function onLightTweenCompleteLeft(tween:FlxTween)
	{
		FlxTween.tween(gambleLight, {x: gambleLight.x - 290, angle: 9}, 2.5, {ease: FlxEase.sineInOut, onComplete: onLightTweenCompleteRight});
	}

	function onLightTweenCompleteRight(tween:FlxTween)
	{
		FlxTween.tween(gambleLight, {x: gambleLight.x + 290, angle: -9}, 2.5, {ease: FlxEase.sineInOut, onComplete: onLightTweenCompleteLeft});
	...
	function moveCameraSection(?id:Int = 0):Void
	{
		...
		{
			if (curSong == "pasta-night")
				camFollow.set((((gf.getMidpoint().x - boyfriend.getGraphicMidpoint().x) / 2) + boyfriend.getGraphicMidpoint().x) + camDisplaceX, gf.getMidpoint().y + 100 + camDisplaceY);
			else 
				camFollow.set(gf.getMidpoint().x, gf.getMidpoint().y - 100);
			//tweenCamIn();
			return;
		}
	...
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
				...
				if (luaModchart != null)
				{
					camDisplaceX += luaModchart.getVar("followXOffset", "float");
					camDisplaceY += luaModchart.getVar("followYOffset", "float");
			...
			camFollow.set(curDad.getMidpoint().x + 150 + camDisplaceX, curDad.getMidpoint().y - 100 + camDisplaceY);
					
			switch (dad.curCharacter)//camera repositioning per character
			{
				case 'mom':
		...
		}else 
		{
			...
			camFollow.set(curBF.getMidpoint().x - 100 + camDisplaceX, curBF.getMidpoint().y - 100 + camDisplaceY);

			if (SONG.song.toLowerCase() == "tutorial")
				FlxTween.tween(FlxG.camera, {zoom: 1}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.elasticInOut});

			switch (curStage)//camera repositioning per stage
			{
				case 'limo':
			...
			switch (boyfriend.curCharacter)//camera repositioning per boyfriend
			{
				case 'bfTT':
	...
	private function onKeyPress(pressArray:Array<Bool>):Void 
	{
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		/*notes.forEachAlive(function(daNote:Note)
		{
			...
			!CoolThings.downscroll && daNote.y < strumLine.y)
			{
				...
				{
					if (daNote.noteType != 3)// hurt note
					{
						goodNoteHit(daNote);
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			{
				curBF.playAnim(animToPlay, true);
				curBF.holdTimer = 0;
			}
	...
	function cpuNoteHit(note:Note):Void
	{
		...
		if (dad.curCharacter != "xgaster")
		{
			...
			{
				if (!noAnimate)
					curDad.playAnim(animToPlay + altAnim, true);
				curDad.holdTimer = 0;
			}
		}else 
		{
			if (animToPlay == "singLEFT" || animToPlay == "singUP")
			{
				dad.visible = true;
				dad2.visible = false;
	...
	function characterPlayAnim(char:Character, anim:String)
	{
		noAnimate = true;
		freeAnimation = true;
	...
	function preloadCharacter(charName:String)
	{
		var bitch = new Character(0, 0, charName);
		bitch.alpha = 0.00001;
		add(bitch);
		preloadedCharacters++;
	}
	...
	function switchCharacter(player:String, switchTo:String, keepChar:Bool = true)
	{
		var daX:Float;
		var daY:Float;
		...
		switch (SONG.song.toLowerCase())
		{
			case "challengedd": 
				switch (curStep)
				{
					case 928: 
						curDad = dad2;
						curDad.playAnim("well", true);
						defaultCamZoom = 0.9;
						dad.playAnim("turn");
						new FlxTimer().start(0.5, function(timer:FlxTimer)
						{
							switchCharacter("dad", "edd-alt", false);
							dad.flipX = !dad.flipX;
						});
					...
					case 944: 
						defaultCamZoom = 0.8;
					case 1008 | 1232 | 1360 | 1488: 
						curBF = dad;
					case 1136 | 1296 | 1424 | 1552:
						curBF = boyfriend;
				}
			...
			case "golden-land":
				if (dad.curCharacter != "MarioLand" && curStage == "mario-land")
				{
					goldenLandP2BG.alpha = 1;
					goldenLandP2Floor.alpha = 1;
					goldenLandP2Floor2.alpha = 1;
				} 
		}
	...
	override function beatHit()
	{
		...
		FlxG.watch.addQuick("ghjfgh", Conductor.songPosition);
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case "i-hate-you":
				if (marioDancer1.poppedOut)
					marioDancer1.dance();
				if (marioDancer2.poppedOut)
					marioDancer2.dance();
			...
			case "rush-e":
				if (Conductor.songPosition > 127000 && Conductor.songPosition < 137000)
				{
					if (daAngle)
					{
						FlxG.camera.angle = 6;
		...
		if(curBeat % 2 == 0) 
		{
			...
			if (gf.animation.curAnim.name != null && !gf.animation.curAnim.name.startsWith("sing") && !gf.curCharacter.startsWith("gf"))
			{
				gf.dance();
			}
		}
		...
		if (curBeat % gfSpeed == 0 && gf.curCharacter.startsWith("gf"))
		{
			gf.dance();
		}
